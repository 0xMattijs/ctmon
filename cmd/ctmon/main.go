...
func runCmd(args []string) error {
	...
	var (
		...
		writers   = fs.Int("writers", pipeline.DefaultWriters, "concurrent store writers")
		backfill  = fs.Duration("backfill", 10*time.Second, "how often to take hosts off the pending queue (0 disables)")
		bfBatch   = fs.Int("backfill-batch", pipeline.DefaultBackfillBatch, "maximum hosts leased per sweep")
